...
function conky_analog_clock()
    ...
    if (tonumber(hours_24) < 12) then
	    ...
	    r, g, b, a = hex_to_rgba(settings.day_hour_hand.hex, settings.hour_mark.alpha)
    end
    cairo_set_source_rgba(cr, r, g, b, a)
    cairo_set_line_width(cr, glow_width )
    cairo_move_to(cr, xc, yc)
    cairo_line_to(cr, xc + glow_length * math.sin(hours_angle), yc - glow_length * math.cos(hours_angle))
    cairo_stroke(cr)
    if (tonumber(hours_24) < 12) then
	    r, g, b, a = hex_to_rgba(settings.day_hour_hand.hex, settings.day_hour_hand.alpha)
    else
	    r, g, b, a = hex_to_rgba(settings.night_hour_hand.hex, settings.night_hour_hand.alpha)
    end
    cairo_set_source_rgba(cr, r, g, b, a)
    cairo_set_line_width(cr, hour_width)
    cairo_move_to(cr, xc, yc)
